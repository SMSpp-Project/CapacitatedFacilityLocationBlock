 ...
 * - Transportation costs encode distances (already raised to power ell) between
 * scenarios
 * - The objective minimizes the \f$\ell\f$-th power of the Wasserstein distance
 * between the original and reduced probability distributions
 *
 * ### Requirements
 *
 ...
 * scenario reduction problem is typically solved once to select a
 * representative subset, and the implemented heuristics are already efficient
 * enough that incremental updates would provide minimal benefit. All
 * Modifications are cleared at the beginning of each compute() call, and the
 * problem is solved from scratch.
 *
 ...
 * Multiple threads can safely call compute() and parameter methods
 * concurrently.
 *
 * @note For exact MILP-based scenario reduction, attach a MILPSolver to the
 *       CapacitatedFacilityLocationBlock instead of using this heuristic
 * solver.
 */
 ...
 enum int_par_type_SRS {
  ...
  intUseWarmstart = intLastAlgPar +
    3 ,                ///< Enable warm start for local search (0=false, 1=true)
 ...
 enum vint_par_type_SRS {
  vintWarmstartIndices =
    vintLastAlgPar ,  ///< Custom warm start indices (empty = use Dupacova)
 ...
 /** @brief Sets the Block to be solved and initializes internal data
  * structures.
  *
  ...
  *
  * @throws std::invalid_argument if block is not a
 ...
 /** @brief Refreshes cached data from the Block and validates problem
  * structure.
  *
  ...
  * Supported names: "intAlgorithm", "intShuffle", "intRandomSeed",
  * "intUseWarmstart"
  *
  * @param name parameter name as string
  * @return parameter index or Inf<idx_type>() if not recognized
  */
 [[nodiscard]] idx_type int_par_str2idx( const std::string & name ) const
 override;
  ...
  * @return parameter index or Inf<idx_type>() if not recognized
  */
 [[nodiscard]] idx_type dbl_par_str2idx( const std::string & name ) const
 override;
  ...
  * @return parameter index or Inf<idx_type>() if not recognized
  */
 [[nodiscard]] idx_type vint_par_str2idx(
 ...
 std::unique_ptr< DVector >
 normalized_weights;     ///< normalized probabilities if needed
  ...
  */
 void validate_warmstart_indices(
