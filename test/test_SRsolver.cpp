...
TestResult test_parameter_management( ) {
 try {
  ...
  solver3.set_par( ScenarioReductionSolver::vintWarmstartIndices ,
                   std::move( test_indices ) );

  const auto & retrieved_indices = solver3.get_vint_par(
  ...
  if( retrieved_indices.size() != 3 ||
   ...
   return {
    false , "get_vint_par failed to retrieve warm start indices correctly"
   };
  }
 ...
 try {
    ...
    for( int i = 0 ; i < block->get_NFacilities() ; ++i ) {
     ...
     if( val > 0.5 ) { // x should be 0 or 1
      if( customer_assigned ) {
       ...
       return {
        false ,
        "Customer " + std::to_string( j ) + " assigned to multiple facilities"
       };
      }
      ...
      if( ! reduced[ i ] ) {
       ...
       return {
        false ,
        "Customer " + std::to_string( j ) + " assigned to unselected facility "
        + std::to_string( i )
       };
      }
     }
     ...
     if( reduced[ i ] && i != assigned_facility ) {
      ...
      if( other_cost < assignment_cost - 1e-6 ) {
       ...
       return {
        ...
        std::to_string( assigned_facility ) + " with cost " + std::to_string(
         assignment_cost ) +
        " but facility " + std::to_string( i ) + " has cost " + std::to_string(
         other_cost ) + ")"
       };
      }
     }
  ...
  // Part 3: Algorithm comparison (Baseline vs Dupacova)
  {
   ...
   if( baseline_obj < dupacova_obj - 1e-6 ) {
    delete block;
    return {
     false , "Baseline objective (" + std::to_string( baseline_obj ) +
  ...
  // Part 6: Warm start with vintWarmstartIndices
  {
   ...
   solver2.set_par( ScenarioReductionSolver::vintWarmstartIndices ,
   ...
   if( obj2 > obj1 + 1e-6 ) {
    ...
    delete block;
    return {
     false , "Warm start local search should not be worse than Dupacova"
    };
   }
  ...
  // Part 7: has_var_solution() and get_var_solution() behavior
  {
   ...
   if( ! solver.has_var_solution() ) {
    delete block;
    return {
     false , "has_var_solution() should return true after successful compute()"
    };
   }
   ...
   if( ! approx_equal( weight_sum , 1.0 , 1e-6 ) ) {
    ...
    return {
     false ,
     "Weights should be normalized to sum to 1.0, got " + std::to_string(
      weight_sum )
    };
   }
   ...
   for( int algo = 0 ; algo <= 3 ; ++algo ) {
    ...
    for( CapacitatedFacilityLocationBlock::Index i = 0 ; i < block->
         get_NFacilities() ; ++i ) {
     auto y_var = block->get_y( i );
    ...
    if( count_selected != 2 ) { // Should select exactly k=2 facilities
     delete block;
     return {
      false , "Algorithm " + std::to_string( algo ) + " selected " +
 ...
 try {
  // Part 1: Concurrent parameter setting
  {
   ...
   if( errors > 0 ) {
    return {
     false ,
     "Concurrent parameter setting had " + std::to_string( errors ) + " errors"
    };
   }
  }
  ...
  // Part 2: Preventing simultaneous compute() calls
  {
   ...
   if( completed != 5 ) {
    return {
     false , "Not all threads completed compute(): " +
  ...
  // Part 3: Thread safety during set_Block()
  {
   ...
   if( errors > 0 ) {
    return {
     false , "Errors during concurrent set_Block: " + std::to_string( errors )
    };
   }
  }
 ...
 try {
  ...
  // Part 3: Weight normalization
  {
   ...
   if( ! solver2.has_var_solution() ) {
    delete block2;
    return {
     false , "Should have solution after compute with normalized weights"
    };
   }
...
REGISTER_TEST( "Test 5 - BlockSolverConfig Deserialization" ,
