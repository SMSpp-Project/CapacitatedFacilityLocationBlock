...
ScenarioReductionSolver::ScenarioReductionSolver( )
 : nb_atoms( 0 ) , nb_reduced( 0 ) , weights( nullptr ) ,
 f_transportation_costs( nullptr ) , f_solution_value( 0 ) {}
...
void ScenarioReductionSolver::set_Block( Block * block ) {
 ...
 if( f_Block ) {
  ...
  if( ! cfl_block ) {
   throw(std::invalid_argument( "ScenarioReductionSolver only works "
    "with CapacitatedFacilityLocationBlock" ));
  }

  // Lock the block if not owned
  bool owned = f_Block->is_owned_by( f_id );
  if( ! owned && ! f_Block->lock( f_id )) {
   throw(std::runtime_error( "ScenarioReductionSolver: unable to "
    "lock the Block" ));
  }
...
int ScenarioReductionSolver::compute( bool changedvars ) {
 ...
 // typically used once to select a representative subset. Reoptimization based
 // on Modifications is not considered as the entire problem is expected to be
 // solved from scratch each time. The algorithms are already efficient enough
 // that incremental updates would provide minimal benefit for the added
 // complexity.
 mod_clear( );
 ...
 BLOG(
   0 ,
   "ScenarioReductionSolver: reducing "
   << nb_atoms << " scenarios to " << nb_reduced << " using "
   << (algorithm == Algorithm::Baseline   ? "Baseline":
   algorithm == Algorithm::Dupacova ? "Dupacova":
   algorithm == Algorithm::BestFit  ? "BestFit":
   "FirstFit")
   << " algorithm" << std::endl );
...
int ScenarioReductionSolver::compute_dupacova( ) {
 ...
 BLOG(
   1 ,
   "\n"
   << "  " << std::setw( 6 ) << "Iter" << " | " << std::setw( 10 ) << "Added"
   << "\n"
   << "  " << std::string( 6 , '-' ) << "-+-" << std::string( 10 , '-' )
   << std::endl );
 ...
 for(int k = 0; k < m; ++k) {
  ...
  // Updates
  for(int i = 0; i < n; i++) {
   minimum_d[ i ] = std::min( minimum_d[ i ] , (*f_transportation_costs)[ i ][
    j_best ] );
  }
  ...
  BLOG(
    1 ,
    "  " << std::setw( 6 ) << (k + 1) << " | " << std::setw( 10 )
         << ("Scen " + std::to_string( j_best )) << std::endl );
 }
 ...
 double dot_product = std::inner_product(
  ...
  weights->begin( ) ,
  0.0 );
 f_solution_value = dot_product;
 // Already the ell-th power of Wasserstein distance
...
int ScenarioReductionSolver::compute_baseline( ) {
 ...
 std::sort(
   weight_index_pairs.begin( ) ,
   weight_index_pairs.end( ) ,
 ...
 for(Index i = 0; i < nb_atoms; ++i) {
  ...
  for(Index j = 0; j < nb_atoms; ++j) {
   if( reduced_atoms[ j ] ) {
    min_distances[ i ] =
      std::min( min_distances[ i ] , (*f_transportation_costs)[ i ][ j ] );
   }
  }
 }

 double total_distance = std::inner_product(
  min_distances.begin( ) ,
  min_distances.end( ) ,
  weights->begin( ) ,
  0.0 );
 ...
 BLOG(
   0 ,
   "  Baseline: selected " << nb_reduced << " highest probability scenarios"
                           << ", objective = " << f_solution_value
                           << std::endl );
...
int ScenarioReductionSolver::compute_local_search( ) {
 ...
 BLOG(
   1 ,
   "\n"
   << "  " << std::setw( 6 ) << "Iter" << " | " << std::setw( 15 )
   << "Objective" << " | " << std::setw( 12 ) << "Improvement" << " | "
   << std::setw( 20 ) << "Action" << "\n"
   << "  " << std::string( 6 , '-' ) << "-+-" << std::string( 15 , '-' )
 ...
 while( improvement ) {
  ...
  if( algorithm == Algorithm::BestFit ) {
   ...
   throw(std::invalid_argument( "compute_local_search() only works with "
  ...
  if( i >= 0 && j >= 0 && trial_d < curr_d ) {
   ...
   BLOG(
     1 ,
     "  " << std::setw( 6 ) << local_search_iterations << " | " << std::setw( 15
     )
          << std::fixed << std::setprecision( 6 ) << trial_d << " | "
          << std::setw( 12 ) << std::fixed << std::setprecision( 6 )
          << improvement_val << " | " << "Swap " << i << " -> " << j
          << std::endl );
   ...
   BLOG(
     ...
     "  " << std::setw( 6 ) << local_search_iterations << " | " << std::setw( 15
          ...
          << std::fixed << std::setprecision( 6 ) << curr_d << " | "
          << std::setw( 12 ) << "No improve" << " | " << "Converged"
          << std::endl );
   ...
   BLOG(
     ...
     "\n  " << (algorithm == Algorithm::BestFit ? "BestFit" : "FirstFit")
            ...
            << ", final objective = " << std::fixed << std::setprecision( 6 )
            << curr_d << std::endl );
  }
 }
...
double ScenarioReductionSolver::init_local_search( ) {
 ...
 double initial_dist = std::inner_product(
  min_distances.begin( ) ,
  min_distances.end( ) ,
  weights->begin( ) ,
  0.0 );
 ...
 if( use_warmstart ) {
  if( ! warmstart_indices.empty( )) {
   BLOG(
     0 ,
     "  Local search initialized with custom warm start indices"
     << ", initial objective = " << initial_dist << std::endl );
  }
  else {
   BLOG(
     0 ,
     "  Local search initialized with Dupacova warm start"
     << ", initial objective = " << initial_dist << std::endl );
  }
 }
 else {
  BLOG(
    0 ,
    "  Local search initialized with random selection"
    << ", initial objective = " << initial_dist << std::endl );
 }
...
std::tuple<
  ScenarioReductionSolver::Index ,
  ScenarioReductionSolver::Index ,
  double >ScenarioReductionSolver::pick_ij_bestfit( double & curr_d ) {
...
ScenarioReductionSolver::IndexDistancePair ScenarioReductionSolver::
bestfit_selection( const std::vector< Index > & curr_indices ) {
 int n = static_cast< int >(nb_atoms);

 // Calculate minimum distances to current reduced set
 std::vector< double > min_on_ind_red( n , std::numeric_limits< double >::
   infinity( ));
 for(int i = 0; i < n; ++i) {
  for(auto j : curr_indices) {
   min_on_ind_red[ i ] =
     std::min( min_on_ind_red[ i ] , (*f_transportation_costs)[ i ][ j ] );
  }
 }
 ...
 for(auto j : indices_to_choose) {
  ...
  double obj_val = std::inner_product(
   combined_min.begin( ) ,
   combined_min.end( ) ,
   weights->begin( ) ,
   0.0 );
...
std::tuple<
  ScenarioReductionSolver::Index ,
  ScenarioReductionSolver::Index ,
  double >ScenarioReductionSolver::pick_ij_firstfit( double & curr_d ) {
 // If shuffling is enabled, shuffle the reduced set
 if( shuffle ) { std::shuffle( ind_red.begin( ) , ind_red.end( ) , rng ); }
 ...
 for(auto i : ind_red) {
  ...
  for(auto idx : ind_red) {
   if( idx != i ) { temp_indices.push_back( idx ); }
  }
  ...
  if( j != static_cast< Index >(-1)) { return { i , j , dist }; }
 }

 // If we need to restore the original order of ind_red
 if( shuffle ) { std::sort( ind_red.begin( ) , ind_red.end( )); }
...
ScenarioReductionSolver::IndexDistancePair ScenarioReductionSolver::
firstfit_selection( const std::vector< Index > & curr_indices , double curr_d )
{
 int n = static_cast< int >(nb_atoms);
 ...
 for(auto j : indices_to_choose) {
  ...
  // Calculate objective value
  double trial_d = std::inner_product(
   combined_min.begin( ) ,
   combined_min.end( ) ,
   weights->begin( ) ,
   0.0 );
  ...
  if( trial_d < curr_d ) { return { j , trial_d }; }
 }
...
std::tuple< int , int > ScenarioReductionSolver::pick_dupacova(
 ...
 // For every atom j in indices_to_choose, compute Wasserstein distance with
 // closed formula
 ...
 auto min_it = std::min_element( distances.begin( ) , distances.end( ));
 int j_tmp = std::distance( distances.begin( ) , min_it );
 // index in indices_to_choose
...
void ScenarioReductionSolver::swap_indices( Index i , Index j ) {
 ...
 auto it_j = std::find( indices_to_choose.begin( ) , indices_to_choose.end( ) ,
  j );
...
void ScenarioReductionSolver::get_var_solution( Configuration * solc ) {
 ...
 if( ! f_Block ) {
  throw(std::logic_error( "ScenarioReductionSolver::get_var_solution: "
   "no Block set" ));
 }
 ...
 auto cfl_block = dynamic_cast< CapacitatedFacilityLocationBlock * >(f_Block);
 if( ! cfl_block ) {
  throw(std::logic_error( "ScenarioReductionSolver::get_var_solution: Block is "
   "not CapacitatedFacilityLocationBlock" ));
 }

 // Check that abstract representation exists
 if( ! cfl_block->get_y( 0 )) {
  throw(std::logic_error(
   "ScenarioReductionSolver::get_var_solution: variables "
   "not generated in Block" ));
 }
 ...
 // For each customer (scenario), find the closest open facility
 // (representative) and set x[facility][customer] = 1.0 for that assignment
  ...
  for(Index customer = 0; customer < nb_atoms; ++customer) {
   ...
   for(Index facility = 0; facility < nb_atoms; ++facility) {
    ...
    if( x_var ) {
     ...
     x_var->set_value(
       (facility == best_facility && reduced_atoms[ facility ]) ? 1.0 : 0.0 );
    }
   }
  }
 }
...
bool ScenarioReductionSolver::has_var_solution( ) {
 ...
 return(
        (f_solution_value >= 0) && ! reduced_atoms.empty( ) &&
        std::any_of( reduced_atoms.begin( ) , reduced_atoms.end( ) , [](bool val
        ) {
  return val;
 } ));
}
...
void ScenarioReductionSolver::set_par( idx_type par , std::vector< int > &&
  value ) {
 switch( par ) {
...
const std::vector< int > &ScenarioReductionSolver::get_vint_par(
  idx_type par ) const {
...
int ScenarioReductionSolver::get_dflt_int_par( idx_type par ) const {
 ...
 case intAlgorithm:
  return 1; // Dupacova
 case intShuffle:
  return 0; // No shuffling
 case intRandomSeed:
  return 0; // Default seed
 case intUseWarmstart:
  return 0; // No warm start by default
 default:
...
Solver::idx_type ScenarioReductionSolver::int_par_str2idx(
  const std::string & name ) const {
 if( name == "intAlgorithm" ) return intAlgorithm;

 if( name == "intShuffle" ) return intShuffle;

 if( name == "intRandomSeed" ) return intRandomSeed;

 if( name == "intUseWarmstart" ) return intUseWarmstart;
...
Solver::idx_type ScenarioReductionSolver::dbl_par_str2idx(
  const std::string & name ) const {
 // No double parameters specific to ScenarioReductionSolver
...
Solver::idx_type ScenarioReductionSolver::vint_par_str2idx(
  const std::string & name ) const {
 if( name == "vintWarmstartIndices" ) return vintWarmstartIndices;
...
void ScenarioReductionSolver::refresh_cached_data( int k ) {
 ...
 if( ! f_Block ) {
  throw(std::logic_error(
   "ScenarioReductionSolver::refresh_cached_data: no Block set" ));
 }

 // Cast to the specialized type
 auto cfl_block = dynamic_cast< CapacitatedFacilityLocationBlock * >(f_Block);
 if( ! cfl_block ) {
  throw(std::logic_error( "ScenarioReductionSolver::refresh_cached_data: Block "
   "is not CapacitatedFacilityLocationBlock" ));
 }

 // Validate k parameter
 if( k < 0 ) {
  throw(std::invalid_argument(
   "ScenarioReductionSolver::refresh_cached_data: k must be non-negative" ));
 }
 ...
 // Validate that we have a square distance matrix (customers == facilities for
 // scenario reduction)
 if( nb_atoms != nb_reduced ) {
  throw(std::logic_error(
   "ScenarioReductionSolver::refresh_cached_data: for scenario reduction, "
 ...
 if( k > nb_atoms ) {
  throw(std::invalid_argument(
   "ScenarioReductionSolver::refresh_cached_data: k "
   "cannot exceed number of scenarios" ));
 }
 ...
 if( original_demands.empty( )) {
  throw(std::runtime_error( "ScenarioReductionSolver::refresh_cached_data: "
   "Block has no demand data" ));
 }

 if( capacities.empty( )) {
  throw(std::runtime_error( "ScenarioReductionSolver::refresh_cached_data: "
   "Block has no capacity data" ));
 }

 if( ! f_transportation_costs ) {
  throw(std::runtime_error( "ScenarioReductionSolver::refresh_cached_data: "
   "Block has no transportation cost data" ));
 }

 // Define tolerance for floating point comparisons
 const double tolerance = 1e-6;

 // For scenario reduction, capacities should all be 1.0 to allow sending all
 // mass to a single facility
 if( k > 0 ) {
  for(size_t i = 0; i < capacities.size( ); ++i) {
   if( std::abs( capacities[ i ] - 1.0 ) > tolerance ) {
    throw(std::invalid_argument(
     "ScenarioReductionSolver::refresh_cached_data: for scenario reduction, "
     "all capacities must be 1.0. Capacity " +
 ...
 double sum =
   std::accumulate( original_demands.begin( ) , original_demands.end( ) , 0.0 );
...
void ScenarioReductionSolver::validate_warmstart_indices(
 ...
 if( indices.size( ) != static_cast< size_t >(m)) {
  throw(std::invalid_argument(
   "ScenarioReductionSolver::validate_warmstart_indices: warmstart_indices "
   "size (" +
 ...
 for(Index idx : indices) {
  if( idx < 0 || idx >= n ) {
   throw(std::invalid_argument(
    ...
    std::to_string( idx ) + " out of bounds [0, " + std::to_string( n - 1 ) +
    "]" ));
  }
  if( ! seen.insert( idx ).second ) {
   throw(std::invalid_argument(
    "ScenarioReductionSolver::validate_warmstart_indices: duplicate "
    "warmstart index " +
    std::to_string( idx )));
  }
 }
