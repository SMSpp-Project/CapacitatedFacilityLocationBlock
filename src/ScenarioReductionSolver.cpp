...
ScenarioReductionSolver::ScenarioReductionSolver( )
 : nb_atoms( 0 ), nb_reduced( 0 ),
   weights( nullptr ), f_transportation_costs( nullptr ),
   f_solution_value( 0 ) {
}
...
int ScenarioReductionSolver::compute_dupacova( ) {
 ...
 for( int k = 0 ; k < m ; ++k ) {
  ...
  for( int i = 0 ; i < n ; i++ ) {
   minimum_d[ i ] = std::min( minimum_d[ i ] ,
                              ( *f_transportation_costs )[ i ][ j_best ] );
  }
 ...
 double dot_product = std::inner_product( minimum_d.begin() , minimum_d.end() ,
                                          weights->begin() , 0.0 );
...
double ScenarioReductionSolver::init_local_search( ) {
 ...
 double initial_dist = std::inner_product( min_distances.begin() ,
                                           min_distances.end() ,
                                           weights->begin() , 0.0 );
...
std::tuple< ScenarioReductionSolver::Index , ScenarioReductionSolver::Index ,
            double >
...
ScenarioReductionSolver::bestfit_selection(
 ...
 std::vector< double > min_on_ind_red(
  n , std::numeric_limits< double >::infinity() );
 for( int i = 0 ; i < n ; ++i ) {
  for( auto j : curr_indices ) {
   min_on_ind_red[ i ] = std::min( min_on_ind_red[ i ] ,
                                   ( *f_transportation_costs )[ i ][ j ] );
  }
 }
 ...
 for( auto j : indices_to_choose ) {
  ...
  for( int i = 0 ; i < n ; ++i ) {
   combined_min[ i ] = std::min( min_on_ind_red[ i ] ,
                                 ( *f_transportation_costs )[ i ][ j ] );
  }

  // Calculate objective value
  double obj_val = std::inner_product( combined_min.begin() ,
                                       combined_min.end() , weights->begin() ,
                                       0.0 );
...
ScenarioReductionSolver::firstfit_selection(
 ...
 for( auto j : indices_to_choose ) {
  ...
  // Calculate objective value
  double trial_d = std::inner_product( combined_min.begin() ,
                                       combined_min.end() , weights->begin() ,
                                       0.0 );
...
std::tuple< int , int > ScenarioReductionSolver::pick_dupacova(
 ...
 for( size_t idx = 0 ; idx < indices_to_choose.size() ; idx++ ) {
  ...
  for( int i = 0 ; i < nb_atoms ; i++ ) {
   inner_min[ i ] = std::min( min_cost[ i ] ,
                              ( *f_transportation_costs )[ i ][ j ] );
  }
  distances[ idx ] = std::inner_product( inner_min.begin() , inner_min.end() ,
                                         weights->begin() , 0.0 );
 }
...
void ScenarioReductionSolver::get_var_solution( Configuration * solc ) {
 ...
 if( ! cfl_block ) {
  throw( std::logic_error(
   "ScenarioReductionSolver::get_var_solution: Block is not CapacitatedFacilityLocationBlock" ) );
 }

 // Check that abstract representation exists
 if( ! cfl_block->get_y( 0 ) ) {
  throw( std::logic_error(
   "ScenarioReductionSolver::get_var_solution: variables not generated in Block" ) );
 }
...
void ScenarioReductionSolver::set_par( idx_type par ,
...
void ScenarioReductionSolver::refresh_cached_data( int k ) {
 ...
 if( ! cfl_block ) {
  throw( std::logic_error(
   "ScenarioReductionSolver::refresh_cached_data: Block is not CapacitatedFacilityLocationBlock" ) );
 }
 ...
 if( k > nb_atoms ) {
  throw( std::invalid_argument(
   "ScenarioReductionSolver::refresh_cached_data: k cannot exceed number of scenarios" ) );
 }
 ...
 if( original_demands.empty() ) {
  throw( std::runtime_error(
   "ScenarioReductionSolver::refresh_cached_data: Block has no demand data" ) );
 }

 if( capacities.empty() ) {
  throw( std::runtime_error(
   "ScenarioReductionSolver::refresh_cached_data: Block has no capacity data" ) );
 }

 if( ! f_transportation_costs ) {
  throw( std::runtime_error(
   "ScenarioReductionSolver::refresh_cached_data: Block has no transportation cost data" ) );
 }
 ...
 double sum = std::accumulate( original_demands.begin() ,
                               original_demands.end() , 0.0 );
